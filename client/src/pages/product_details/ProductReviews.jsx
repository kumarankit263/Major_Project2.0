  ...
  const [isReviewFirstTimeLoading, setIsReviewFirstTimeLoading] =
    ...
    useEffect(() => {
      const getReview = async () => {
        try {
          let response = await getReviews(productData?._id, currentPage);
    
          // ✅ Ensure response is valid and extract the correct array
          const reviewsArray = response?.reviews || [];
    
          if (!Array.isArray(reviewsArray)) {
            console.error("Error: Expected an array, but got:", reviewsArray);
            return;
          }
          ...
          if (reviewsArray.length === 0) {
            ...
            // ✅ Avoid duplicate reviews (check for existing ones)
            setReviewData((prevReviews) => {
              const newReviews = reviewsArray.filter(
                (newReview) => !prevReviews.some((prev) => prev._id === newReview._id)
              );
